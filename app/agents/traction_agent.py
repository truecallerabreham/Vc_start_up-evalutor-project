...
def _score(hits: list[HybridHit]) -> float:
    ...
    return max(1.0, min(10.0, round(3.5 + (avg * 6.2), 2)))
...
def run_traction_agent(startup_name: str, hits: list[HybridHit]) -> MemoSection:
    summary = (
        f"{startup_name} has early traction indicators from news/site mentions. "
        "Validate customer retention, revenue quality, and pipeline conversion before investment."
    )
